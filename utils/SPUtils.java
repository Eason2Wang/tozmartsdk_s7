...
public final class SPUtils {
    ...
    public static SPUtils getInstance(String spName, Context context) {
        return getInstance(spName, context, Context.MODE_PRIVATE);
    }
     ...
     * @return the single {@link SPUtils} instance
     */
    public static SPUtils getInstance(String spName, Context context, final int mode) {
        if (isSpace(spName)) spName = "spUtils";
        SPUtils spUtils = SP_UTILS_MAP.get(spName);
        if (spUtils == null) {
            synchronized (SPUtils.class) {
                spUtils = SP_UTILS_MAP.get(spName);
                if (spUtils == null) {
                    spUtils = new SPUtils(spName, context, mode);
                    SP_UTILS_MAP.put(spName, spUtils);
                }
            }
        }
    ...
    private SPUtils(final String spName) {
        sp = OneMeasureSDKLite.getInstance().getApplicationContext().getSharedPreferences(spName, Context.MODE_PRIVATE);
    }

    private SPUtils(final String spName, final int mode) {
        sp = OneMeasureSDKLite.getInstance().getApplicationContext().getSharedPreferences(spName, mode);
    }

    private SPUtils(final String spName, Context context, final int mode) {
